...
export default async function WinningsPage() {
  ...
  const totalWon = winners?.filter((w) => w.payment_status === 'paid').reduce((s, w) => s + w.prize_amount_cents, 0) ?? 0;
  const pending = winners?.filter((w) => w.verification_status === 'approved' && w.payment_status === 'pending').reduce((s, w) => s + w.prize_amount_cents, 0) ?? 0;
