...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: existingDraw } = await adminSupabase.from('draws').select('id').eq('draw_month', validated.draw_month).single();
    ...
    const { data: previousDraw } = await adminSupabase.from('draws').select('match_5_pool_cents, rollover_cents, match_5_count').lt('draw_month', validated.draw_month).in('status', ['published', 'completed']).order('draw_month', { ascending: false }).limit(1).single();
