...
export function WinningsOverview({ winners }: { winners: Winner[] }) {
  ...
  const totalWon = winners.filter((w) => w.payment_status === 'paid').reduce((sum, w) => sum + w.prize_amount_cents, 0);
  const pendingAmount = winners.filter((w) => w.verification_status === 'approved' && w.payment_status === 'pending').reduce((sum, w) => sum + w.prize_amount_cents, 0);
